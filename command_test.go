...
func TestGrep_Extended_InvalidPattern(t *testing.T) {
	// An unterminated group is not a valid regular expression. -E must surface
	// the ErrInvalidPattern sentinel — with the compile failure recoverable as
	// its cause — rather than panic or silently match nothing. The pattern fails
	// to compile before any input is read, so the error is reported even when
	// the input is empty.
	tests := []struct {
		wantErr error
		name    string
		...
		{name: "with input", input: "abc\n", wantErr: command.ErrInvalidPattern},
		{name: "empty input", input: "", wantErr: command.ErrInvalidPattern},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := testable.TestLines(command.Grep("a(b", command.GrepExtended), tt.input)
			if !errors.Is(err, tt.wantErr) {
				t.Fatalf("got %v, want errors.Is %v", err, tt.wantErr)
			}
			var cause *syntax.Error
			if !errors.As(err, &cause) {
				t.Fatalf("got %v, want the regexp compile failure recoverable as the cause", err)
			}
		})
	}
}

func TestGrep_UnknownOptionPassesThrough(t *testing.T) {
	// An argument that is not one of grep's flag types is handed to the
	// framework's positional classifier and must not disturb matching.
	lines, err := testable.TestLines(command.Grep("hello", 42), "hello\nworld\n")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
